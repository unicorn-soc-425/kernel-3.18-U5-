		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (global_rt5659->impedance_gain_map == true) {
		reg05 = ucontrol->value.integer.value[0];
		reg06 = ucontrol->value.integer.value[1];

		dev_info(codec->dev, "%s  %d Impedance value\n",__func__, rt5659->impedance_value);
		if (reg05 > global_rt5659->impedance_gain) {
			reg05 = reg05 - global_rt5659->impedance_gain;
		} else {
			reg05 = 0x0;
		}
		if (reg06 > global_rt5659->impedance_gain) {
			reg06 = reg06 - global_rt5659->impedance_gain;
		} else {
			reg06 = 0x0;
		}
	...
	if (global_rt5659->impedance_gain_map == true)
		snd_soc_update_bits(codec, RT5659_BIAS_CUR_CTRL_8, 0x0700,
			global_rt5659->impedance_bias << 8);
...
void rt5659_imp_detect_jd_unplug()
{
	...
	global_rt5659->impedance_gain = 1;
	global_rt5659->impedance_bias = 5;
}
...
unsigned int rt5659_imp_detect(struct snd_soc_codec *codec)
{
	...
	dev_info(codec->dev, "%s HP Imp Detection start!\n", __func__);
	...
	snd_soc_update_bits(codec, RT5659_ASRC_2, 0x7000, 0);
	...
	snd_soc_update_bits(codec, RT5659_PWR_DIG_1, 0x0c00, 0);
	...
	snd_soc_write(codec, RT5659_STO_NG2_CTRL_1, 0x0ec0);
	snd_soc_write(codec, RT5659_HP_CHARGE_PUMP_1, 0x0c16);
	snd_soc_write(codec, RT5659_CAL_REC, 0x0505);
	snd_soc_update_bits(codec, RT5659_PWR_ANLG_1, 0xa200, 0xa200);
	...
	for (j = 0; j < 3; j++) {
		...
		for (i = 0; i < 30; i++) {
			...
			if (!(snd_soc_read(codec, RT5659_HP_IMP_SENS_CTRL_1) & 0x8000)) {
...
imp_break:
	...
	for (i = 0; i < ARRAY_SIZE(hp_gain_table); i++) {
		if (rt5659->impedance_value < hp_gain_table[i].min
		 || rt5659->impedance_value > hp_gain_table[i].max)
			continue;

		dev_info(codec->dev, "SET GAIN %d for %d Impedance value\n",
				 hp_gain_table[i].gain, rt5659->impedance_value);
		rt5659->impedance_gain = hp_gain_table[i].gain;
		rt5659->impedance_bias = hp_gain_table[i].bias;		
	}
	...
	dev_info(codec->dev, "%s HP Imp Detection done value=%d gain=%d \n",
		__func__, rt5659->impedance_value, rt5659->impedance_gain);

	return rt5659->impedance_value;
}
EXPORT_SYMBOL(rt5659_imp_detect);
...
static const char *rt5659_jack_type_mode[] = {
	"Disable", "Read"
};
...
<<<<<<< HEAD
	...
	SND_SOC_DAPM_SUPPLY("DAC Mono Right Filter", RT5659_PWR_DIG_2,
		RT5659_PWR_DAC_MF_R_BIT, 0, rt5659_monor_filter_event,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
		SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMD),
=======
...
<<<<<<< HEAD
	SND_SOC_DAPM_MUX("DAC MIXL Mux", RT5659_DIG_MIXER, RT5659_M_DAC_MIX_L_SFT,
		1, &rt5659_dig_dac_mixl_mux),
	SND_SOC_DAPM_MUX("DAC MIXR Mux", RT5659_DIG_MIXER, RT5659_M_DAC_MIX_R_SFT,
		1, &rt5659_dig_dac_mixr_mux),
=======
...
<<<<<<< HEAD
	{ "I2S1", NULL, "I2S1 ASRC", rt5659_i2s_use_asrc },
	{ "I2S2", NULL, "I2S2 ASRC", rt5659_i2s_use_asrc },
	{ "I2S3", NULL, "I2S3 ASRC", rt5659_i2s_use_asrc },
...
<<<<<<< HEAD
	{ "Stereo1 ADC L1 Mux", "DAC MIX", "DAC MIXL Mux" },
	{ "Stereo1 ADC L2 Mux", "DMIC", "Stereo1 DMIC L Mux" },
	{ "Stereo1 ADC L2 Mux", "DAC MIX", "DAC MIXL Mux" },

	{ "Stereo1 ADC R1 Mux", "ADC", "Stereo1 ADC R Mux" },
	{ "Stereo1 ADC R1 Mux", "DAC MIX", "DAC MIXR Mux" },
	{ "Stereo1 ADC R2 Mux", "DMIC", "Stereo1 DMIC R Mux" },
	{ "Stereo1 ADC R2 Mux", "DAC MIX", "DAC MIXR Mux" },
=======
...
<<<<<<< HEAD
	{ "Mono ADC Volume L", NULL, "Mono ADC MIXL" },
	{ "Mono ADC Volume R", NULL, "Mono ADC MIXR" },

	{ "IF_ADC1", NULL, "Stereo1 ADC Volume L" },
	{ "IF_ADC1", NULL, "Stereo1 ADC Volume R" },
	{ "IF_ADC2", NULL, "Mono ADC Volume L" },
	{ "IF_ADC2", NULL, "Mono ADC Volume R" },
=======
	{ "IF_ADC1", NULL, "Stereo1 ADC Volume L" },
	{ "IF_ADC1", NULL, "Stereo1 ADC Volume R" },
...
<<<<<<< HEAD
	{ "DAC L2 Mux", "Mono ADC MIX", "Mono ADC Volume L" },
=======
...
<<<<<<< HEAD
	{ "DAC R2 Mux", "Mono ADC MIX", "Mono ADC Volume R" },
=======
...
<<<<<<< HEAD
	{ "DAC MIXL Mux", "Stereo DAC Mixer", "Stereo DAC MIXL" },
	{ "DAC MIXL Mux", "Mono DAC Mixer", "Mono DAC MIXL" },
	{ "DAC MIXR Mux", "Stereo DAC Mixer", "Stereo DAC MIXR" },
	{ "DAC MIXR Mux", "Mono DAC Mixer", "Mono DAC MIXR" },
=======
...
<<<<<<< HEAD
		...
		if (dai->id == RT5659_AIF1)
			snd_soc_update_bits(codec, RT5659_ASRC_2,
				RT5659_DA_STO_T_MASK | RT5659_AD_STO1_T_MASK,
				0);
		break;
	case 96000:
		snd_soc_update_bits(codec, RT5659_ADDA_CLK_1, 
	...
	case SND_SOC_BIAS_STANDBY:
		if (codec->dapm.bias_level == SND_SOC_BIAS_OFF) {
			...
			regmap_update_bits(rt5659->regmap, RT5659_PWR_ANLG_1, RT5659_PWR_MB |
				RT5659_PWR_VREF1 | RT5659_PWR_VREF2, RT5659_PWR_MB |
				RT5659_PWR_VREF1 | RT5659_PWR_VREF2);
			msleep(20);
...
static int rt5659_parse_dt(struct rt5659_priv *rt5659, struct device_node *np)
{
	...
	rt5659->pdata.gpio_1p8_en = of_get_named_gpio(np, "realtek,gpio_1p8_en", 0);
	rt5659->pdata.gpio_3p3_en = of_get_named_gpio(np, "realtek,gpio_3p3_en", 0);		

	rt5659->pdata.noise_gate1_hp_enabled = of_property_read_bool(np,
					"realtek,noise-gate1-hp-enabled");

	if (!of_property_read_u32_array(np, "imp_table", data, (len * 4))) {
		...
		rt5659->impedance_gain_map = true;
		rt5659->impedance_value = 0x3ff;
		rt5659->impedance_gain = 0;
		rt5659->impedance_bias = 5;
	} else {
		rt5659->impedance_gain_map = false;
	}
=======
...
<<<<<<< HEAD
	...
	if (gpio_is_valid(rt5659->pdata.gpio_1p8_en)) {
		if (gpio_request(rt5659->pdata.gpio_1p8_en, "rt5659_1p8_en"))
			dev_err(&i2c->dev, "Fail gpio_request rt5659_1p8\n");
		else if (gpio_direction_output(rt5659->pdata.gpio_1p8_en, 1))
			dev_err(&i2c->dev, "Fail gpio_direction rt5659_1p8\n");
		else
			dev_err(&i2c->dev, "gpio_direction rt5659_1p8 is success\n");
	}
	if (gpio_is_valid(rt5659->pdata.gpio_3p3_en)) {
		if (gpio_request(rt5659->pdata.gpio_3p3_en, "rt5659_3p3_en"))
			dev_err(&i2c->dev, "Fail gpio_request rt5659_3p3\n");
		else if (gpio_direction_output(rt5659->pdata.gpio_3p3_en, 1))
			dev_err(&i2c->dev, "Fail gpio_direction rt5659_3p3\n");
		else
			dev_err(&i2c->dev, "gpio_direction rt5659_3p3 is success\n");			
	}

	if (gpio_is_valid(rt5659->pdata.gpio_ldo)) {
		if (gpio_request(rt5659->pdata.gpio_ldo, "rt5659_ldo"))
			dev_err(&i2c->dev, "Fail gpio_request gpio_ldo\n");
		else if (gpio_direction_output(rt5659->pdata.gpio_ldo, 1))
			dev_err(&i2c->dev, "Fail gpio_direction gpio_ldo\n");
		else
			dev_err(&i2c->dev, "gpio_direction gpio_ldo is success\n");			
	}

	if (gpio_is_valid(rt5659->pdata.gpio_reset)) {
		if (gpio_request(rt5659->pdata.gpio_reset, "rt5659_reset"))
			...
			dev_err(&i2c->dev, "gpio_direction gpio_reset is success\n");			
	}
...
<<<<<<< HEAD
		...
		dev_info(&i2c->dev,
			"Device with ID register %x is rt5659\n", val);
...
<<<<<<< HEAD
	...
	if (gpio_is_valid(rt5659->pdata.gpio_1p8_en)) {
		gpio_direction_output(rt5659->pdata.gpio_1p8_en, 0);
		gpio_free(rt5659->pdata.gpio_1p8_en);
	}
	
	if (gpio_is_valid(rt5659->pdata.gpio_3p3_en)) {
		gpio_direction_output(rt5659->pdata.gpio_3p3_en, 0);
		gpio_free(rt5659->pdata.gpio_3p3_en);
	}		
